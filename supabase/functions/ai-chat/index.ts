...
// Knowledge base for fallback when OpenAI is not available
const kutalbeKnowledgeBase = {
  'what is kutable': 'Kutable is a professional barber booking platform that connects customers with barbers. Customers can find and book barbers online, while barbers can claim profiles, manage bookings, and accept payments. The platform is mobile-first with SMS confirmations and reminders included.',
  
  'how to book': 'To book an appointment: 1) Browse our barber directory and find a barber you like, 2) Select a service and available time slot, 3) Complete booking with secure payment, 4) Receive instant SMS confirmation. You\'ll also get a reminder 24 hours before your appointment.',
  
  'cancel appointment': 'You can cancel appointments up to 24 hours in advance directly from your dashboard for a full refund. For rescheduling or cancellations within 24 hours, please contact your barber directly using their phone number on their profile.',
  
  'payment pricing': 'Payments are processed securely through Stripe when you book. There are no additional booking fees for customers. You pay the service price set by the barber. All payments are secure and encrypted.',
  
  'barber fees': 'Barbers pay only when they earn - there\'s a 4% total fee per transaction which includes 1% platform fee and approximately 3% payment processing fee. No monthly subscriptions, setup fees, or contracts. Money goes directly to your bank account.',
  
  'claim profile': 'Barbers can find their business in our directory and claim it for free. After claiming, you can set your services, pricing, availability, and connect your bank account to start accepting online payments. No monthly fees or setup costs.',
  
  'sms notifications': 'After booking, you\'ll receive SMS confirmations and 24-hour reminders. You can opt out anytime by replying STOP to any message. Make sure to provide a valid phone number during booking to receive notifications.',
  
  'support help': 'For platform and technical issues, email support@kutable.com. For appointment questions (changes, directions, etc.), contact your barber directly using their phone number on their profile. We respond to support emails within 24 hours.',
  
  'privacy security': 'Your data is secure with bank-level encryption. We never sell your information and only use it to provide services. You can manage your communication preferences in your dashboard.',
  
  'find barbers': 'Browse our verified directory of professional barbers. You can filter by location, rating, services, and availability. All barbers show reviews from real customers and you can see their portfolios and pricing.'
};

const findBestMatch = (query: string): { answer: string; confidence: number } => {
  const queryLower = query.toLowerCase();
  let bestMatch = '';
  let bestScore = 0;
  
  Object.entries(kutalbeKnowledgeBase).forEach(([keywords, answer]) => {
    const keywordList = keywords.split(' ');
    let score = 0;
    
    keywordList.forEach(keyword => {
      if (queryLower.includes(keyword)) {
        score += keyword.length / queryLower.length;
      }
    });
  ...
  return { answer: bestMatch, confidence: bestScore };
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check if we have OpenAI API key for advanced features
    if (!openaiApiKey || openaiApiKey.trim() === '' || 
        openaiApiKey === 'your_openai_api_key_here' || 
        openaiApiKey.includes('placeholder')) {
      
      logEvent('warn', `[${requestId}] OpenAI API key not configured, using fallback knowledge base`);
      
      // Fallback to simple keyword matching
      const match = findBestMatch(sanitizedMessage);
      
      if (match.confidence > 0.1) {
        aiResponse = match.answer + '\n\n💡 For more detailed help, email support@kutable.com or visit our support page.';
        logEvent('info', `[${requestId}] Fallback match found`, {
          confidence: match.confidence,
          responseLength: aiResponse.length
        });
      } else {
        aiResponse = `I'm here to help with questions about Kutable! You can ask me about:

• How to book appointments
• Barber fees and pricing
• Claiming your barber profile
• Payment and security
• SMS notifications
• Platform features

For specific technical support, please email support@kutable.com

What would you like to know about Kutable?`;
        
        logEvent('info', `[${requestId}] No fallback match found, using default response`);
      }
    } else {
      // Use OpenAI if API key is available
      logEvent('info', `[${requestId}] OpenAI API key available, using advanced AI features`);
      ...
      // Initialize OpenAI and generate embedding
      let openai;
      try {
        ...
        const match = findBestMatch(sanitizedMessage);
        if (match.confidence > 0.1) {
          aiResponse = match.answer + '\n\n💡 AI service temporarily unavailable. For detailed help, email support@kutable.com';
        } else {
          aiResponse = 'I apologize, but the AI service is temporarily unavailable. For immediate assistance, please email support@kutable.com or visit our support page.';
        }
        
        logEvent('info', `[${requestId}] Used fallback response due to OpenAI init failure`);
      }

      if (openai && !aiResponse) {
        ...
        try {
          ...
          logEvent('info', `[${requestId}] Used fallback response due to embedding error`);
        }

        if (embeddingResponse && !aiResponse) {
          ...
          if (relevantDocs && relevantDocs.length > 0) {
            ...
            logEvent('info', `[${requestId}] No relevant context found, using fallback`);
            // If no database context found, try keyword matching
            const match = findBestMatch(sanitizedMessage);
            if (match.confidence > 0.1) {
              context = match.answer;
              logEvent('info', `[${requestId}] Used keyword fallback context`);
            }
          }
...
User Question: ${sanitizedMessage}`;
          ...
          try {
            ...
            if (match.confidence > 0.1) {
              ...
              aiResponse = 'I apologize, but the AI service is temporarily unavailable. For immediate assistance, please email support@kutable.com or visit our support page for common questions.';
            }
            
            logEvent('info', `[${requestId}] Used fallback response due to completion error`);
          }

          if (completion && !aiResponse) {
            aiResponse = completion.choices[0]?.message?.content || 'I apologize, but I was unable to generate a response. Please try again or contact our support team at support@kutable.com';
    ...
    // Save conversation messages (only if we have a database connection)
    if (supabase && conversation && !aiResponse.includes('AI service temporarily unavailable')) {
    ...
    const responseData = {
      ...
      sources: relevantDocs && relevantDocs.length > 0 ? relevantDocs.slice(0, 2) : [],
      timestamp: new Date().toISOString(),
      requestId,
      mode: openaiApiKey && openaiApiKey.trim() !== '' ? 'ai' : 'fallback'
    };
