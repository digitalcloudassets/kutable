...
interface Props {
  refreshAdminKpis: () => Promise<void>;
}

const BookingsManagement: React.FC<Props> = ({ refreshAdminKpis }) => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      const { data, error } = await supabase
        .from('bookings')
        .select('*, client_profiles(*), services(*)')
  ...
  const processRefund = async (booking: Booking) => {
    ...
    try {
      const res = await fetch('/api/admin/refund', {
        ...
        body: JSON.stringify({ payment_intent: booking.stripe_payment_intent_id }),
      });
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const matchSearch = `${b.client_profiles?.first_name || ''} ${b.client_profiles?.last_name || ''}`
      .toLowerCase()
              ...
              <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-gray-200">
                ...
                {(booking.status === 'confirmed' || booking.status === 'completed') && booking.stripe_payment_intent_id && (
                  <button
                    ...
                    className="px-4 py-2 bg-red-500 text-white rounded-lg text-sm font-medium hover:bg-red-600 disabled:opacity-50 flex items-center space-x-2"
                  >
